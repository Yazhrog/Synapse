...
- Testing a QML change: **`config/quickshell` is not copied anywhere** — Hyprland launches quickshell directly against the repo path (`quickshell -c ~/.local/src/Synapse/config/quickshell`, see `config/hypr/modules/autostart.lua`). So editing files under `config/quickshell/` and restarting quickshell (`pkill quickshell` then re-trigger autostart, or `hyprctl dispatch exit` to relaunch Hyprland) is enough to see the change — no install step needed.
...
1. **Copied at install time**: `config/hypr/`, `config/ghostty/`, `config/mpv/`, `config/fastfetch/`, `config/starship/`, `config/zsh/` are copied into `~/.config/...` by `install/steps/05-config.sh` and `06-shell-config.sh`. Edits here require reinstalling/re-copying to take effect on a live system.
2. **Run in place from the repo**: `config/quickshell/` is *not* copied — Quickshell is launched with `-c` pointing straight at `~/.local/src/Synapse/config/quickshell`. Only the wallpaper assets under `config/quickshell/src/assets/wallpapers/` get seeded into `~/Pictures/Wallpapers/`.
