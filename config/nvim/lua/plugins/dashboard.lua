return {
    ...
    config = function()
        ...
        dashboard.section.buttons.val = {
            button("e", icons.kinds.custom_icons.treesitter .. " Nvim Tree", function()
            ...
            button("o", icons.ui.restore .. " Restore Session", [[<cmd> lua require("persistence").load() <CR>]]),
        ...
        local greeting = function()
            ...
            elseif datetime >= 18 and datetime < 22 then
        ...
        vim.api.nvim_create_autocmd({ "User" }, {
            ...
            callback = function()
                if vim.fn.executable("onefetch") == 1 then
                    local header = split(
                        capture(
                            [[onefetch --true-color always 2>/dev/null | sed 's/\x1B[@A-Z\\\]^_]\|\x1B\[[0-9:;<=>?]*[-!"#$%&'"'"'()*+,.\/]*[][\\@A-Z^_`a-z{|}~]//g']],
                            true
                        ),
                        "\n"
                    )
