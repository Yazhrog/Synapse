...
# Locally-patched copy of the wayland-idle-inhibitor-git AUR script: upstream
# imports pywayland's protocol bindings via the old per-interface submodule
# layout, but current python-pywayland generates one flat module per protocol,
# so upstream raises ModuleNotFoundError on launch. Vendored here instead of
# relying on the AUR package being fixed upstream.
